...
npm run dev
```
Backend runs at `http://localhost:5000`. Confirm it's up by visiting `http://localhost:5000/api/health`.
...
npm run dev
```
Frontend runs at `http://localhost:5173` and proxies all `/api/*` requests to the backend automatically (see `vite.config.js`).

### 4. Log in
Use the credentials set in `setSeededPasswords.js` for Michael (Manager) or Daryl (Warehouse). Sales Rep accounts are created afterward by the Manager through the Employees page.
